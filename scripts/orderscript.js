...
function modifyQty() {
    ...
    ext.value = parseInt(this.value, 10) * Math.round((parseFloat(document.getElementById(this.getAttribute("id") + "price").value, 10) + Number.EPSILON) * 100) / 100
        ;
    ttl.value = 0;
    let product_TTLs = document.getElementsByClassName("productexts");
    var total = 0;
    for (var i = 0; i < products_order.length; i++) {
        total += Math.round((parseFloat(product_TTLs[i].value, 10) + Number.EPSILON) * 1000) / 1000;
    }
    ttl.value = total;
}